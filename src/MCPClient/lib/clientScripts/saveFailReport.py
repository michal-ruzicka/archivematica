...
# @author Michal Ruzicka <ruzicka@ics.muni.cz>
import smtplib
...
def saveEmail(subject, to, from_, content, dir):
    to2 = ", ".join(to) 
    ...
    filename = "%s.%s.%s.%s" % ('FailReport', datetime.now().strftime("%Y-%m-%dT%H-%M-%S%z"), str(uuid.uuid4()), 'msg')
...
if __name__ == '__main__':
    ...
    from_ = "ArchivematicaSystem@archivematica.ics.muni.cz"
    content = getContentFor(opts.unitType, opts.unitName, opts.unitIdentifier)
    dir = "/var/archivematica/fail-reports/"
    
    saveEmail(subject, to, from_, content, dir)
